...
.DESCRIPTION
    This script disables Basic authentication for the Windows Remote Management
    client by creating the required registry policy path and setting AllowBasic
    to 0.
...
Write-Host "Starting remediation for STIG WN11-CC-000330..."
...
# Create or update the AllowBasic DWORD value
...
if ($result.$valueName -eq 0) {
    Write-Host "Validation Passed: $valueName is set to $($result.$valueName). WinRM Client Basic authentication is disabled."
} else {
    Write-Host "Validation Failed: $valueName is set to $($result.$valueName)."
}

Write-Host "STIG WN11-CC-000330 remediation completed."