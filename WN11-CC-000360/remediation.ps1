...
.DESCRIPTION
    This script disables Digest authentication for the Windows Remote Management
    client by creating the required registry policy path and setting AllowDigest
    to 0.
...
Write-Host "Starting remediation for STIG WN11-CC-000360..."
...
# Create or update the AllowDigest DWORD value
...
if ($result.$valueName -eq 0) {
    Write-Host "Validation Passed: $valueName is set to $($result.$valueName). WinRM Client Digest authentication is disabled."
} else {
    Write-Host "Validation Failed: $valueName is set to $($result.$valueName)."
}

Write-Host "STIG WN11-CC-000360 remediation completed."